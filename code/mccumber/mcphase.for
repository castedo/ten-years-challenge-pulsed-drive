...
CC $DEFINE graphout        ! DEFINEd for graphical output
c
...
CC      OPEN (UNIT = 10, FILE =filename//'.dat', STATUS = 'OLD')
...
c ----------------------------------------------------------------------
      ...
      LOGICAL        twoway
CC       INTEGER*2      iyr, imon, iday
CC       INTEGER*2      ihr, imin, isec, dummy
      character*8    date
      character*10   time
      character*5    zone
      integer        values(8)
...
CC       OPEN (UNIT = 11, ACCESS='APPEND', FILE = filename//'.out',
CC      +      STATUS = 'UNKNOWN')
      OPEN (UNIT = 11, ACCESS='APPEND', FILE = TRIM(filename)//'.out',
     +      STATUS = 'UNKNOWN')

      WRITE (11, '(A)') line1

CC       WRITE (11, '(A, A)') 'Filename: ', filename
CC       CALL GETDAT(iyr, imon, iday)
CC       WRITE (11, '(A, I2.2, A, I2.2, A, I4)')
CC      +      'Date:     ', iday, '.', imon, '.', iyr
CC       CALL GETTIM(ihr, imin, isec, dummy)
CC       WRITE (11, '(3(A, I2.2))')
CC      +      'Time:     ', ihr, ':', imin, ':', isec
      call date_and_time(date, time, zone, values)
C       write (11, '(A, 2X, A, 2X, A)') date, time, zone
C       write (11, '(8i5)') values
      iyr  = values(1)
      imon = values(2)
      iday = values(3)
      ihr  = values(5)
...
C      CALL gettim(hr, min, sec, dummy)
      call date_and_time(date, time, zone, values)
      hr = values(5)
      min = values(6)
      sec = values(7)
